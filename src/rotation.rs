...
use crate::crd::{
    ...
    Provider, TargetResult, TargetStatus,
};
...
impl Rotator {
    ...
    async fn rotate_inner(
        ...
        let probe = if req.skip_verification {
            ...
            match self.prober.probe(provider, &req.value).await {
                ...
                Err(e) => {
                    ...
                    warn!(key = name, actor = %req.actor, error = %e, "probe failed; nothing written");
                    self.repo
                        .record_event(
                            &key,
                            AuditEvent {
        ...
        self.repo
            ...
            .await;
        info!(key = name, actor = %req.actor, completed, failed = ?failed, "key submitted");
